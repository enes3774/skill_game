...
GameServer.prototype.updateMoveEngine = function() {
    ...
    for (var i = 0; i < len; i++) {
        ...
        for (var j = 0; j < list.length ; j++) {
            ...
            // 🆕 CRYPTO: Handle kill rewards (SOL transfer)
...
// ========================================
// 🆕 CRYPTO: Betting System Functions
// ========================================

/**
 * Handle kill reward - Transfer SOL from victim to killer
 */
GameServer.prototype.handleKillReward = function(killer, victim, eatenMass) {
    // Calculate total mass for each player
    var killerTotalMass = this.getTotalMass(killer);
    var victimTotalMass = this.getTotalMass(victim);
    
    if (victimTotalMass <= 0 || !victim.monetaryValue) return;
    
    // Calculate transfer amount based on mass eaten
    var transferRatio = eatenMass / victimTotalMass;
    var transferAmount = victim.monetaryValue * transferRatio;
    
    // Apply rake (5% on kills)
    var rakePercentage = parseFloat(process.env.RAKE_PERCENTAGE || '0.05');
    var rake = transferAmount * rakePercentage;
    var netTransfer = transferAmount - rake;
    ...
    // Log transaction to database (async, don't wait)
    if (this.db) {
        this.db.createTransaction({
            ...
            gameId: this.gameId
        }).catch(err => console.error('Failed to log kill transaction:', err));
        
        this.db.createTransaction({
            userId: 'house',
            type: 'rake',
            amount: rake
        }).catch(err => console.error('Failed to log rake:', err));
    }
    
    console.log(`💰 Kill: ${killer.name} +${netTransfer.toFixed(4)} SOL from ${victim.name} (rake: ${rake.toFixed(4)})`);
    
    // Check if victim is eliminated (no cells left)
...
GameServer.prototype.handlePlayerDeath = function(player) {
    ...
    // Log game participation (async)
    if (this.db && player.gameId) {
        this.db.query(
        ...
        ).catch(err => console.error('Failed to update game participation:', err));
        
        // Update user balance
        this.db.updateUserBalance(player.privyId, player.monetaryValue)
...
GameServer.prototype.broadcastMonetaryUpdate = function(player) {
    // Send updated monetary value to player
    if (player.socket && player.socket.sendPacket) {
        var Packet = require('./packet');
        var packet = new Packet.ServerMsg('Your balance: ' + player.monetaryValue.toFixed(4) + ' SOL');
        player.socket.sendPacket(packet);
    }
};
